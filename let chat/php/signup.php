...
// let check user emeil is valid or not
if(filter_var($email, FILTER_VALIDATE_EMAIL)){ // if email is valid
// let check if the email exit in the database or not
 $sql = mysqli_query($conn, "SELECT email FROM users WHERE email ='{$email}'");
 if(mysqli_num_rows($sql) >0){ //if email already exist
    echo "$email - This email already exist!";
 }else{
    //lets's check user upload file or not
    if(isset($_FILES['image'])){ //if file is uploaded
        ...
        $extensions = ['png', 'jpeg', 'jpg', 'PNG','JPEG','JPG' ]; // these are some valid img ext and we've store them in array
        if(in_array($img_ext, $extensions) === true){ // if user uploaded img ext is matched with any array extensions
        $time = time(); //this will return us current time.. 
                       // we need this time becouse when you uploading user img to in our folder we rename user file with current time 
                       // so all the time imag will have  a unique name
            ...
            echo "please select an image file - jpeg, jpg, png, JPnG, JPG, PNG!";
        }
 }else{
